...
class Supervisor():
    ...
    def _get_criteria(self):
        prompt = PromptTemplate(input_variables=['input'], 
                template=(self.get_success_criteria)
                )
        criteria_chain= LLMChain(llm=self._llm, prompt=prompt,output_key="node_tasks",verbose=True)        
        return(criteria_chain)
    
    def _eval_plan_chain(self):
        prompt = PromptTemplate(input_variables=['tasks','criteria','goal'], 
                template=(self.eval_tasks)
                )
        eval_plan_chain= LLMChain(llm=self._llm, prompt=prompt,output_key="node_tasks",verbose=True)        
        return(eval_plan_chain)

    def _eval_design(self):
        prompt = PromptTemplate(input_variables=['task','criteria','goal'], 
                template=(self.eval_task)
                )
        eval_plan_chain= LLMChain(llm=self._llm, prompt=prompt,output_key="node_tasks",verbose=True)        
        return(eval_plan_chain)
    ...
    def _eval_task(self):
        prompt = PromptTemplate(input_variables=['tasks','current_task'], 
                template=(self.eval_task)
                )
        eval_plan_chain= LLMChain(llm=self._llm, prompt=prompt,output_key="node_tasks",verbose=True)        
        return(eval_plan_chain)
